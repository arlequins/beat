...
export const addPostListEpic: TEpic = (action$: ActionsObservable<Action<typeof POST_LIST_REQUEST>>) =>
  action$.ofType(POST_LIST_REQUEST).pipe(
    mergeMap((action: PostListRequestAction) => (
      defer(() => requestPostList(action.payload))
        .pipe(
          map((postList: PostList) => setPostList(postList)),
          catchError((error: any): Observable<PostListFailureAction> => (
            of(errorPostList(error))
          ))
        )
    ))
  )
