...
const BlogTopPart: React.FC<AllProps> = () => {
  const { postList = {
    ...
    result: [] as Section[],
  }, route = {} as RouteConfig, sideBar = {} as FetchedSideBar } = useSelector((state: State) => state)
  ...
  const featuredPosts = postList.result.featuredPosts.filter((featuredPost: FeaturedPostInterface) => featuredPost.type !== 1)
