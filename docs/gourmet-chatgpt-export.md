...
1. Beat `/admin/`에 Google SSO로 로그인한 탭을 열고 관리자 기록 목록이 보이는지 확인한다.
2. 사진이 있는 ChatGPT 대화에서 확장 버튼을 누른다.
3. 확장은 관리자 로그인 상태를 먼저 진단한 뒤, 현재 페이지에 로드된 사용자
   메시지의 이미지 묶음을 읽고, 기존 Beat `draft` 기록과 식당명·메뉴·요약·평점을
   비교해 연결 후보를 만든다.
4. 후보와 사진 미리보기를 확인한다. 관련 없는 사진은 체크를 해제하고
   연결할 초안도 확인한 뒤, **확인한 사진을 초안에 연결**을 눌러 체크된 사진만
   전송한다.
...
- 이미 같은 초안에 연결된 사진은 콘텐츠 해시로 확인해 다시 업로드하지 않는다.
...
- `Beat Admin 탭을 ... 열어 주세요`: 같은 Chrome 프로필에서
  `https://arlequins.github.io/beat/admin/`를 연다.
- `Beat Admin 탭을 새로고침해 주세요`: 확장을 설치한 뒤 관리자 탭을 새로고침해
  세션 브리지를 다시 로드한다.
- `로그인 세션이 만료되었습니다`: 관리자 탭에서 Google SSO를 다시 완료한다.
- `현재 화면에 로드된 사진이 없습니다`: 사진이 보이는 메시지까지 스크롤해
  썸네일을 로드한 뒤 다시 찾는다.
- `사진은 보이지만 직접 올린 사용자 메시지에서 찾지 못했습니다`: 공유 링크나
  ChatGPT가 생성한 이미지는 지원 대상이 아니므로, 원본 대화에서 직접 첨부한
  사용자 메시지를 연다.
